                    ...
                    try:
                        if modo == "online":
                            ...
                            if reconocedor_offline:
                                fuente.rewind()
                                with wave.open(archivo_audio, 'rb') as wf:
                                    wf.setpos(int(offset * wf.getframerate()))
                                    frames_a_leer = int(duracion * wf.getframerate())
                                    audio_data = wf.readframes(frames_a_leer)
...
# Nueva función para detectar el sistema operativo y configurar adecuadamente
    def detectar_y_configurar_audio_sistema(self):
        sistema = platform.system()
        ...
        if sistema == "Windows":
            ...
            self.configurar_audio_macos()
        elif sistema == "Linux":
            self.configurar_audio_linux()
        else:
            messagebox.showwarning("Sistema no soportado",
                            f"La captura de audio del sistema no está soportada en {sistema}.")
    ...
    def verificar_instalacion(self):
        ...
        if problemas:
            ...
            if not vosk_disponible:
                recomendaciones.append("Instale Vosk para habilitar el modo offline: pip install vosk")
            
            if vosk_disponible and not self.modelos_vosk:
                recomendaciones.append("Descargue modelos desde https://alphacephei.com/vosk/models")
            
            if not encriptacion_disponible:
                recomendaciones.append("Instale Cryptography para habilitar encriptación: pip install cryptography")
            
            if not pydub_disponible:
                recomendaciones.append("Instale pydub para soporte de más formatos: pip install pydub")
    ...
    def instalar_dependencias(self):
        ...
        try:
            ...
            ventana_instalacion = tk.Toplevel(self.root)
            ventana_instalacion.title("Instalando Dependencias")
            ventana_instalacion.geometry("500x300")
            
            frame_instalacion = ttk.Frame(ventana_instalacion, padding=10)
            frame_instalacion.pack(fill=tk.BOTH, expand=True)
            
            texto_salida = tk.Text(frame_instalacion, wrap=tk.WORD, height=15, font=("Courier New", 10))
            texto_salida.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
            
            scrollbar_salida = ttk.Scrollbar(texto_salida, command=texto_salida.yview)
            scrollbar_salida.pack(side=tk.RIGHT, fill=tk.Y)
            texto_salida.config(yscrollcommand=scrollbar_salida.set)
            ...
            def ejecutar_comando(comando):
                ...
                try:
                    ...
                    for linea in proceso.stdout:
                        texto_salida.insert(tk.END, linea)
                        texto_salida.see(tk.END)
                        ventana_instalacion.update()
                    
                    for linea in proceso.stderr:
                        texto_salida.insert(tk.END, f"ERROR: {linea}")
                        texto_salida.see(tk.END)
                        ventana_instalacion.update()
                    ...
                    if proceso.returncode == 0:
                        ...
                        texto_salida.insert(tk.END, f"\n✗ Error en el comando (código {proceso.returncode})\n")
            ...
            def iniciar_instalacion():
                texto_salida.insert(tk.END, "Iniciando instalación de dependencias...\n")
                texto_salida.insert(tk.END, "Este proceso puede tardar varios minutos.\n\n")
                
                paquetes = []
                
                if not vosk_disponible:
                    paquetes.append("vosk")
                
                if not encriptacion_disponible:
                    paquetes.append("cryptography")
                
                if not pydub_disponible:
                    paquetes.append("pydub")
                
                if not docx_disponible:
                    paquetes.append("python-docx")
                ...
                texto_salida.insert(tk.END, "Es necesario reiniciar la aplicación para aplicar los cambios.\n")
                
                boton_cerrar.config(state=tk.NORMAL)
                
                if messagebox.askyesno(
                    "Reiniciar aplicación", 
                    "Es necesario reiniciar la aplicación para aplicar los cambios. ¿Desea reiniciar ahora?"
                ):
                    ventana_instalacion.destroy()
                    self.root.destroy()
            
            frame_botones = ttk.Frame(frame_instalacion)
            frame_botones.pack(fill=tk.X)
            
            boton_cerrar = ttk.Button(
                frame_botones,
                text="Cerrar",
                command=ventana_instalacion.destroy,
                state=tk.DISABLED
            )
            boton_cerrar.pack(side=tk.RIGHT)
            
            threading.Thread(target=iniciar_instalacion, daemon=True).start()
            
        except Exception as e:
            print(f"Error al instalar dependencias: {e}")
            messagebox.showerror("Error", f"No se pudieron instalar las dependencias: {str(e)}")
